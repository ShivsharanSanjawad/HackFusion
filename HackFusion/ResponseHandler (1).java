            ...
            case AWAITING_ISSUE_SINCE:
                if (update.getMessage().hasText()) {
                    try {
                        ...
                        silent.send("Invalid date format. Please mention only the number (e.g., 20).", chatId);
                    }
                } else {
                    silent.send("Please enter the days as number only format.", chatId);
                }
                break;
            ...
            case AWAITING_IMAGE:
                ...
                if (update.getMessage().hasPhoto()) {
                    ...
                    if (report.getMedia_url().isEmpty()) {
                        silent.send("You must upload at least one photo before finishing.", chatId);
                    } else {
